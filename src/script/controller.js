...
var navModule = angular.module('navModule',[]);
	...
	navModule.directive('navModule', function() {
	  return {
	    restrict: 'AE',
	    replace: true,
	    templateUrl:'./tpls/navModule.html',
...
var list = angular.module('list',[]);
	 ...
	 */
	list.controller('listImg',function($scope){
   		$scope.bg_img = 'src/images/gugu.jpg';
   		$scope.self_bg_img = 'src/images/gugu.jpg';
   		$scope.country_img = 'src/images/country.png';
   		$scope.name="GuGu~";
   		$scope.time="1min age";
   		$scope.topic="#kamsomesamida#";
   		$scope.topic_content="索尼最近的动作貌似非常的多，此前曾给出过A68，蔡司Batis以及24-70mm和85mm GM镜头相关消息。而最近又传出消息称，索尼将会推迟A99 II的发布时间。",
   		$scope.like_num="1988";
   		$scope.fw_num="2014";
   		$scope.type="悬赏";
   		$scope.fw=true;
	...
	list.directive('listInfo', function() {
	  return {
	    restrict: 'AE',
	    templateUrl:'./tpls/listInfo.html',
   		link: function(scope, elem, attrs) {
   		}	
	  };
	});
...
var content = angular.module('content',[]);
	...
	content.directive('contentTitle',function(){
		return {
	    restrict: 'AE',
	    templateUrl:'./tpls/contentDetailTitle.html',
   		link: function(scope, elem, attrs) {
   			$('.nav').hide();
   		}	
	  };
...
var listRight = angular.module('listRight',[]);
	...
	listRight.directive('contentRight',function(){
		return {
	    restrict: 'AE',
	    templateUrl:'./tpls/contentRight.html',
   		link: function(scope, elem, attrs) {
	...
	listRight.directive('selfRight',function(){
		return {
	    restrict: 'AE',
	    templateUrl:'./tpls/selfRight.html',
   		link: function(scope, elem, attrs) {
	...
	listRight.directive('topicRight',function(){
		return {
	    restrict: 'AE',
	    templateUrl:'./tpls/topicRight.html',
   		link: function(scope, elem, attrs) {
...
var hotTopic = angular.module('hotTopic',[]);
	 ...
	 * topicImg
	 */
	hotTopic.controller('hotTopicCtrl',function($scope,$http,$timeout){
		/*$http.get("test.json")
		.success(function(response) {
			$scope.names = response.records;
			$timeout(function() {
			  	$('.c_topic_img').find('li:last').css({width:'130px',['background-image']:'none'}).html('<div class="more"><p>&#xe607;</p><span>More</span></div>');
			},1);
		});*/
		$scope.bg_img = 'src/images/gugu.jpg';
	});
